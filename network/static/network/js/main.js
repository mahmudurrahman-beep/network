 ...
 * 
 * @description
 * Automatically dismisses Django flash messages after timeout with:
 * - Bootstrap 4 compatibility
 * - Fallback for environments without jQuery
 * - Mobile-optimized timing (shorter on mobile)
 * - Prevents double-dismiss
 * 
 * @selector .messages .alert.alert-dismissible
 * @fires DOMContentLoaded
 */
(function() {
  /**
   * Closes an alert element with fade animation
   * @param {HTMLElement} el - Alert element to close
   */
  function closeAlert(el) {
    ...
    // Fallback: manual fade and remove
    el.classList.remove("show");
    setTimeout(() => el.remove(), 300);
  }

  /**
   * Sets up auto-dismiss timers for all alerts
   */
 ...
 * 
 * @description
 * Automatically fades out success alerts (e.g., "Comment added!")
 * after 3 seconds with smooth transition.
 * 
 * @selector .alert-success, .comment-added-alert
 */
(function() {
  setTimeout(() => {
